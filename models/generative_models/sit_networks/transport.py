...
class Transport:
    ...
    def training_losses(
        ...
        if zs is not None:
            model_output, zs_tilde = model_output
        B, *_, C = xt.shape
        assert model_output.size() == (B, *xt.size()[1:-1], C)

        terms = {}
        terms['pred'] = model_output
        # Expose trajectory variables for optional auxiliary losses computed upstream.
        terms['t'] = t
        terms['xt'] = xt
        terms['ut'] = ut
        terms['x1'] = x1
        ...
        if zs is not None:
            ...
            for i, (z, z_tilde) in enumerate(zip(zs, zs_tilde)):
                ...
                for j, (z_j, z_tilde_j) in enumerate(zip(z, z_tilde)):
                    ...
                    proj_loss[i].append(mean_flat(-(z_j * z_tilde_j).sum(dim=-1)))
                proj_loss[i] = th.stack(proj_loss[i], dim=0)
            proj_loss = th.stack(proj_loss, dim=0).sum(dim=0)
            terms['loss'] = terms['loss'] + 0.5 * proj_loss
        return terms
