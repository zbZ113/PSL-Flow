...
def save_decomposed_images(preds, input_tensor, output_img_dir, img_name, lossmodule):
    ...
    e_vis = (e - np.mean(e)) / (np.std(e) + 1e-6)
    e_vis = cv2.normalize(e_vis, None, alpha=0, beta=360, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8U)

    hsv_view = np.stack([e_vis, T, env], axis=2).astype(np.uint8)
    hsv_image = cv2.cvtColor(hsv_view, cv2.COLOR_HSV2RGB)
    save_image(hsv_image, os.path.join(output_img_dir, f"{img_name}_hsv.png"))
