...
class PhysSur(nn.Module):
    def __init__(self, in_ch: int = 4, base_ch: int = 64, p_token_num: int = 4, p_token_dim: int = 64):
        super().__init__()
    ...
    def forward(self, z_tir):
        h = self.encoder(z_tir)
        p_map = self.map_head(h)
        p_token = self.token_head(h).view(z_tir.shape[0], self.p_token_num, self.p_token_dim)
