...
case "${DATASET_NAME}" in
  ...
  *)
    echo "[ERR] Unsupported paper dataset: ${DATASET_NAME}" >&2
    exit 2
    ;;
esac
...
BENCH_ROOT="${BENCH_ROOT:-logs/paper_runs/${DATASET_NAME}/${ROUTE}/${RUN_ID}}"
...
if [[ -z "${RUN_ID_WAS_SET}" && -z "${BENCH_ROOT_WAS_SET}" ]] && is_truthy "${AUTO_RESUME_LATEST}"; then
  latest_root="$(find "logs/paper_runs/${DATASET_NAME}/${ROUTE}" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | sort | tail -n 1 || true)"
...
BASE_DIR="psl_flow/configs/paper"
