...
# Dataset / teacher
export SIT_DATASET="${SIT_DATASET:-DroneVehicle_day}"
export TERB_NET_CKPT="${TERB_NET_CKPT:-checkpoints/physics/${SIT_DATASET}/terb_net/teacher_best.pth}"

# PSL-VAE selection
# Recommended selection from local metrics: epoch 290, best FID, thermal_normalizer ~ 0.9693437814712524
export SIT_VAE_SELECT="${SIT_VAE_SELECT:-epoch}"
export SIT_VAE_EPOCH="${SIT_VAE_EPOCH:-290}"
export SIT_VAE_RUN_DIR="${SIT_VAE_RUN_DIR:-logs/psl_vae/DRONEVEHICLEDAY_lpips01_ft}"
export SIT_VAE_METRICS_CSV="${SIT_VAE_METRICS_CSV:-logs/psl_vae/dronevehicleday_metrics.csv}"
export SIT_VAE_CKPT_DIR="${SIT_VAE_CKPT_DIR:-checkpoints/psl_vae_lpips01/DroneVehicle_day/checkpoints}"

# Output / resume
export SIT_RUN_ROOT="${SIT_RUN_ROOT:-logs/psl_flow/DroneVehicle_day_psl_vae_ep290}"
