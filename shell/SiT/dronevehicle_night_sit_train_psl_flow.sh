...
# Dataset / teacher
export SIT_DATASET="${SIT_DATASET:-DroneVehicle_night}"
export TERB_NET_CKPT="${TERB_NET_CKPT:-checkpoints/physics/${SIT_DATASET}/terb_net/teacher_best.pth}"

# PSL-VAE selection
# Recommended selection from local metrics: epoch 170, best FID, thermal_normalizer ~ 0.9758703708648682
export SIT_VAE_SELECT="${SIT_VAE_SELECT:-epoch}"
export SIT_VAE_EPOCH="${SIT_VAE_EPOCH:-170}"
export SIT_VAE_RUN_DIR="${SIT_VAE_RUN_DIR:-logs/psl_vae/DRONEVEHICLENIGHT_lpips01_ft}"
export SIT_VAE_METRICS_CSV="${SIT_VAE_METRICS_CSV:-logs/psl_vae/dronevehiclenight_metrics.csv}"
export SIT_VAE_CKPT_DIR="${SIT_VAE_CKPT_DIR:-checkpoints/psl_vae_lpips01/DroneVehicle_night/checkpoints}"

# Output / resume
export SIT_RUN_ROOT="${SIT_RUN_ROOT:-logs/psl_flow/DroneVehicle_night_psl_vae_ep170}"
