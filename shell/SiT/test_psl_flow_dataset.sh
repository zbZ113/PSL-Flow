...
if [[ "${SIT_CKPT}" == "auto" ]]; then
  ...
  if [[ "${#CKPTS[@]}" -gt 0 ]]; then
    ...
    echo "[PSL-Flow-Test] Auto ckpt=${SIT_CKPT}"
  else
...
TMP_CFG="$(mktemp "/tmp/psl_flow_test_${SIT_DATASET}_XXXXXX.yml")"
cleanup() {
  rm -f "${TMP_CFG}"
}
trap cleanup EXIT

python - "${TEST_CFG}" "${TMP_CFG}" "${SIT_DATASET}" "${SIT_CKPT}" "${SIT_VAE_CKPT_RESOLVED}" "${SIT_RGB_VAE_CKPT}" "${TERB_NET_CKPT}" "${SIT_THERMAL_NORMALIZER}" <<'PY'
import sys
import yaml

test_cfg, out_cfg, dataset_name, sit_ckpt, vae_ckpt, rgb_vae_ckpt, teacher_ckpt, thermal_normalizer = sys.argv[1:]
...
echo "[PSL-Flow-Test] SIT_DATASET=${SIT_DATASET}"
echo "[PSL-Flow-Test] TEST_CFG=${TEST_CFG}"
echo "[PSL-Flow-Test] TMP_CFG=${TMP_CFG}"
echo "[PSL-Flow-Test] SIT_CKPT=${SIT_CKPT}"
echo "[PSL-Flow-Test] SIT_VAE_SELECT=${SIT_VAE_SELECT}"
echo "[PSL-Flow-Test] SIT_VAE_PICKED_EPOCH=${SIT_VAE_PICKED_EPOCH:-<none>}"
echo "[PSL-Flow-Test] SIT_VAE_METRICS_CSV=${SIT_VAE_METRICS_CSV_RESOLVED:-<none>}"
echo "[PSL-Flow-Test] PSL_VAE_CKPT=${SIT_VAE_CKPT_RESOLVED}"
echo "[PSL-Flow-Test] thermal_normalizer=${SIT_THERMAL_NORMALIZER}"
echo "[PSL-Flow-Test] TERB_NET_CKPT=${TERB_NET_CKPT:-<cfg>}"
echo "[PSL-Flow-Test] SIT_RGB_VAE_CKPT=${SIT_RGB_VAE_CKPT:-<default>}"

if [[ "${DISABLE_WANDB}" == "1" ]]; then
  export WANDB_MODE="disabled"
fi

CMD=(python test.py --config "${TMP_CFG}" --devices "${TEST_DEVICES}" --num-nodes "${TEST_NUM_NODES}" --accelerator "${TEST_ACCELERATOR}" --strategy "${TEST_STRATEGY}" --default-root-dir "${SIT_RUN_ROOT}" --vae-path "${SIT_VAE_CKPT_RESOLVED}")
